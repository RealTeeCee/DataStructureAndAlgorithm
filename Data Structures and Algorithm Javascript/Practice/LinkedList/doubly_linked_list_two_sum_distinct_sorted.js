...
class DoublyLinkedList {
  ...
  //       while (temp !== null) {
  //         if (target - temp.value === currentNode.value) {
  //           arr.push([temp.value, currentNode.value]);
  //         }
  //       }
  //       currentNode = currentNode.next;
  //     }
  ...
  twoSum(target) {
    ...
    let arr = [];
    let val = lead.value;
    while (lead.value !== trail.value) {
      if (target - val === trail.value) {
        arr.push([trail.value, val]);
        lead = lead.next;
        val = lead.value;
        if(val === trail.value) break;
        trail = trail.prev;
      } else if (target - val > trail.value) {
        lead = lead.next;
        val = lead.value;
      } else {
        trail = trail.prev;
