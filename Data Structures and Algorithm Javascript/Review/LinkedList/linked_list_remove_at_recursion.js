...
export default class LinkedList {
  ...
  remove(node, index, firstIndex) {
    if (index === 0) return this.removeFirst();
      ...
      if (firstIndex === index - 1) {
        ...
        return this.remove(node.next, index, firstIndex + 1);
      }
    }
  }
  ...
  deleteNode(node, k) {
    ...
    node.next = this.deleteNode(node.next, k - 1);
    return node;
  }
...
// myLinkedList.remove(myLinkedList.head, 4, 0);
// console.log(myLinkedList.deleteNodeRecursive(myLinkedList.head, 3, 0));
console.log(myLinkedList.deleteNode(myLinkedList.head, 3));
let test = "";
